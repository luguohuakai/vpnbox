...
// 只处理srun_box分支
        if (!preg_match('/srun_box/', $data->ref)) {
            L('you are not srun_box', $this->file_name);
            exit('you are not srun_box');
        }

        $output = '';
//if (is_file($dir . $version)) {
        // 第二次及之后直接pull代码
        $command = "cd {$dir} && /usr/local/bin/git pull";
        $output .= shell_exec($command);
        L($command,$this->file_name);
//} else {
        // 有确认 应该也可以解决
...
// 生成版本信息
        $git_version_commit_num = shell_exec("cd {$dir} && /usr/local/bin/git rev-list HEAD | wc -l | awk '{print $1}'");
        $git_version_hash_min = shell_exec("cd {$dir} && /usr/local/bin/git rev-list HEAD --abbrev-commit --max-count=1");
        $git_version_hash_max = shell_exec("cd {$dir} && /usr/local/bin/git rev-parse HEAD");
